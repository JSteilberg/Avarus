...
map<string, map<string, sf::IntRect>> Atlas::ReadRectsByKey(string element_key) const {
    ...
    for(auto& name_rects : atlas_coords_parser_.GetParseTree().get_child(element_key)) {
        ...
        for(auto& rects : name_rects.second.get_child("rects")) {
            ...
            read_rects_map[name_rects.first.data()][rects.second.get<string>("name")] = rect;
